...
export default function App() {
  ...
  /* マスター管理 */
  const [officeMaster, setOfficeMaster] = useState(() => localStorage.getItem('fukushi_office') || DEFAULT_OFFICE)
  const [staffList, setStaffList] = useState(() => {
    try {
      const s = JSON.parse(localStorage.getItem('fukushi_staffList') || 'null')
  ...
  const [contractorList, setContractorList] = useState(() => {
    try {
      const c = JSON.parse(localStorage.getItem('fukushi_contractorList') || 'null')
  ...
  const updateStaffName = (i, v) => setStaffList((p) => p.map((n, idx) => (idx === i ? v : n)))
  const removeStaffName = (i) => setStaffList((p) => p.filter((_, idx) => idx !== i))
  const addStaffName = () => setStaffList((p) => [...p, ''])

  const updateContractorName = (i, v) => setContractorList((p) => p.map((n, idx) => (idx === i ? v : n)))
  const removeContractorName = (i) => setContractorList((p) => p.filter((_, idx) => idx !== i))
  const addContractorName = () => setContractorList((p) => [...p, ''])

  /* localStorage 永続化 */
  useEffect(() => { localStorage.setItem('fukushi_staff', staff) }, [staff])
  useEffect(() => { localStorage.setItem('fukushi_office', officeMaster) }, [officeMaster])
  useEffect(() => { localStorage.setItem('fukushi_staffList', JSON.stringify(staffList)) }, [staffList])
  useEffect(() => { localStorage.setItem('fukushi_contractorList', JSON.stringify(contractorList)) }, [contractorList])
  useEffect(() => {
  ...
  // 印刷で商品詳細列を出すか（特定福祉用具 かつ 入力 or 表示ONのとき）
  ...
  return (
    ...
    <div className="min-h-full bg-gradient-to-br from-slate-50 to-blue-50/30 print:hidden">
      {/* ヘッダー */}
      <header className="sticky top-0 z-30 bg-blue-600 text-white shadow-md px-4 py-2">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-sm font-bold tracking-wide">売上伝票発行依頼フォーム</h1>
            <p className="text-[10px] text-blue-200">福祉用具・住宅改修</p>
          </div>
      ...
      {showMaster && (
        <div className="max-w-5xl mx-auto px-4 pt-3">
          <div className={`${card} space-y-3`}>
            <p className={sectionTitle}>マスター管理</p>
            <div>
              <label className={fieldLabel}>事業所名</label>
              <input
                type="text"
                value={officeMaster}
            ...
            <div>
              <label className={fieldLabel}>担当者リスト</label>
                ...
                {staffList.map((name, i) => (
                  <div key={i} className="flex gap-1">
                    ...
                    {staffList.length > 1 && (
                      <button
                        type="button"
                        onClick={() => removeStaffName(i)}
                        className="h-8 w-8 rounded-lg text-xs text-red-400 hover:bg-red-50 transition"
                      >
                        ×
                      </button>
                    )}
            ...
            <div>
              <label className={fieldLabel}>施工業者リスト（住宅改修）</label>
              <div className="space-y-1">
                {contractorList.map((name, i) => (
                  <div key={i} className="flex gap-1">
                    <input
                      type="text"
                      value={name}
                      onChange={(e) => updateContractorName(i, e.target.value)}
                      className={`${baseInput} flex-1`}
                      placeholder="施工業者名"
                    />
                    <button
                      type="button"
                      onClick={() => removeContractorName(i)}
                      className="h-8 w-8 rounded-lg text-xs text-red-400 hover:bg-red-50 transition"
                    >
                      ×
                    </button>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  ...
                  >
                    ...
                    {staffList.filter((n) => n.trim()).map((n) => (
    ...
    {/* ── 印刷シート（親の外） ──────────────────── */}
    <div className="hidden print:block p-0 text-[11px]">
      {/* 印刷ヘッダー */}
